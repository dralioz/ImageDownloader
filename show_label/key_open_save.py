# -*- coding: utf-8 -*-
"""
Created on Sat Jul 18 14:35:14 2020
...
def write_xml(folder, objects, tl, br, savedir):
    if not os.path.isdir(savedir):
        os.mkdir(savedir)
        
    for n, image_file in enumerate(os.scandir(folder)):
        ...
        root = etree.fromstring(xml_str)
        xml_str = etree.tostring(root, pretty_print=True)
        save_path = os.path.join(savedir, image_file.name.replace('jpg', 'xml'))
        with open(save_path, 'wb') as temp_xml:
            temp_xml.write(xml_str)
